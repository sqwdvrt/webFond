...
export default function HomePage() {
  return <>
    ...
    <section className="content-band"><div className="container"><SectionHeading number="01" title="Цели, предмет и виды деятельности фонда" intro="Конкретные программы и проекты будут опубликованы после их утверждения фондом" /><span className="project-status project-section-status">Виды деятельности по уставу</span><div className="project-grid project-grid-compact">{projects.map((project, index) => <ProjectCard key={project.description} project={project} compact number={index + 1} />)}</div></div></section>
