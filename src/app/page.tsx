...
export default function HomePage() {
  return <>
    ...
    <section className="content-band"><div className="container"><SectionHeading number="01" title="Цели, предмет и виды деятельности фонда" /><p className="activity-intro">Фонд помогает людям, которым особенно нужна поддержка, и объединяет необходимые для этого усилия и средства.</p><ul className="activity-list" role="list">{projects.map((project) => <li key={project.description}>{project.homepageDescription}</li>)}</ul></div></section>
